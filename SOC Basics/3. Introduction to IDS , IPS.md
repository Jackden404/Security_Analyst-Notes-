...
Intrusion Detection Systems (IDS) and Intrusion Prevention Systems (IPS) form the backbone of network security. While they sound similar, their underlying architecture dictates how they function and fail.
##### 📖 Core Concept: Inline vs. Out-of-band Deployment

The primary distinction between IDS and IPS lies in how they sit in the physical network topology.

**Field Fact:** Which architectural difference is the defining characteristic between IDS and IPS?
**IPS is deployed inline in the traffic path while IDS is deployed out-of-band on a span or tap.**
Because IPS is inline, it can block traffic in real-time. 
Because IDS is out-of-band, it only analyzes a copy of the traffic and alerts passively.

##### 🛡️ Technical Overview: Inline IPS Failure Modes
...
- If the appliance fails without a bypass pair, the network connection is severed. 
- **Takeaway:** **All traffic through that path is dropped, causing an outage.**
...
How do we spot the bad guys? We cannot search for what we do not understand. In this module, we explore the three major detection methodologies: Signature, Anomaly, and Stateful Protocol Analysis.
...
##### 🛡️ Technical Overview: Stateful Protocol Analysis
...
Where you stand determines what you can see. Network Intrusion Detection Systems (NIDS) watch the highways connecting servers, while Host Intrusion Detection Systems (HIDS) stand directly in the servers' rooms.
...
##### 🛡️ Technical Overview: Unique HIDS Capabilities
...
Think of Suricata or Snort as security guards that look at faces and alert you when they match a mugshot. Zeek (formerly Bro) is a surveillance camera network that logs everything everyone does, whether they match a mugshot or not.
...
A SOC analyst's worst enemy is noise. When you are hit with 500 alerts a day, alert triage is the art of separating the harmless firecrackers from the active landmines.
#### 📖 Core Concept: Success vs. Failure

A signature alert only means an exploit _attempt_ was made. It does not mean the attacker got in.
...
#### 🛡️ Technical Overview: Escalating to Endpoint Analysis
...
In security monitoring, false alarms consume valuable resources. Learning how to properly handle false positives (FPs) is critical to maintaining a healthy SOC operation.
...
#### 🛡️ Technical Overview: Deduplication and Rate Limiting
