...
Information is the lifeblood of security operations. Open Source Intelligence (OSINT) is the art and science of finding public needles in the global digital haystack.

**🎯 Mission Objective**

By the end of this module, you will understand what OSINT is, distinguish between passive, semi-active, and active collection, understand legal limits, and learn from historic OSINT investigations.
...
### 🛡️ Technical Overview: Legal Authorization
...
Random Googling is not intelligence. To execute successful OSINT investigations without drowning in raw information, you must adhere to a structured, iterative framework.

🎯 Mission Objective

By the end of this module, you will understand the five phases of the intelligence cycle, how confirmation bias ruins investigations, and the role of the processing phase.
...
### 🛡️ Technical Overview: The Processing Phase
...
One wrong packet can burn a multi-week investigation. Operational Security (OPSEC) is the practice of protecting your identity and actions from target detection.

🎯 Mission Objective

By the end of this module, you will learn the threats of browser fingerprinting, best practices for sock puppet preparation, and advanced anonymization configurations.
...
### 🛡️ Technical Overview: Sock Puppet Lifespans
...
Google has already crawled and indexed the vast majority of the public web. You do not need to scan a target server directly if Google has already indexed its configurations, databases, and exposed documents.


🎯 Mission Objective

By the end of this module, you will understand Google search operators (specifically `site:`), harness the Google Hacking Database (GHDB), and master dorking search workflows.
...
🛡️ Technical Overview: The Google Hacking Database (GHDB)
...
1. `site:example.com`
2. `site:example.com filetype:xls`
3. `site:example.com filetype:xls inurl:budget`
4. `site:example.com filetype:xls inurl:budget intext:"confidential"`

---
# People & Username Recon

Usernames are not just random strings. They are personal identifiers. Because humans are creatures of habit, they reuse handles across dozens of social networks, forums, and developer platforms.

**🎯 Mission Objective**

By the end of this module, you will understand the power of username reuse, utilize historical archives like the Wayback Machine, and master analytical pivot techniques.
...
### 🛡️ Technical Overview: Wayback Machine Investigations
...
An email address is not just a routing header for messages. It is the single most valuable pivot point in OSINT, serving as a unique persistent identifier linking physical people to their digital shadows.

🎯 Mission Objective

By the end of this module, you will understand the power of email pivoting, know the differences between HIBP and DeHashed, and leverage domain pattern discovery with Hunter.io.
...
Every picture tells a story, but digital images tell secrets. Image Intelligence (IMINT) is the discipline of extracting location, device details, time metadata, and visual context from photos and videos.


🎯 Mission Objective

By the end of this module, you will understand the value of raw metadata, identify visual cues for geolocation when metadata is stripped, and perform frame-by-frame video analysis.
...
### 🛡️ Technical Overview: Geolocating Without Metadata
...
A target's online presence relies on infrastructure. Mapping out domains, subdomains, SSL certificates, and DNS records is the first step in assessing a corporate target's attack surface.

🎯 Mission Objective

By the end of this module, you will understand passive vs. active subdomain enumeration, leverage Certificate Transparency logs, and decode TXT DNS records.
...
### 🛡️ Technical Overview: Active vs. Passive Discovery
...
A digital trace eventually leads to a physical coordinate. Geolocating targets involves combining satellite imagery, solar telemetry, and wireless network mappings.

🎯 Mission Objective

By the end of this module, you will learn the importance of convergent evidence, utilize SunCalc for temporal analysis, and leverage Wigle.net for physical positioning.
...
### 🛡️ Technical Overview: Solar Telemetry with SunCalc
...
---
# Module 18 Study Guide: Open Source Intelligence (OSINT)

Before attempting the Module 18 Final Quiz, review this comprehensive study guide. It covers the fundamentals of OSINT, search dorks, username/email recon, image analysis, infrastructure mapping, and risk strategies.

## 

🎯 Phase 1: OSINT Foundations & OPSEC

- **OSINT Definition:** OSINT stands for **Open Source Intelligence**, which represents intelligence gathered strictly from publicly available sources.
    
- **Passive OSINT Legality:** Passive OSINT (accessing public data) is **generally NOT illegal**, as it does not involve unauthorized access or hacking.
    
- **Identity Protection:** The best way to protect your identity during online research is to **use a Sock Puppet and a VPN**.
    
- **Sock Puppet Verification:** You should always verify sock puppet accounts with a burner phone because modern **platforms require SMS verification** during sign-up.
    
- **Active Recon Risk:** The primary risk of active reconnaissance (such as active port scanning or direct target interaction) is that **it generates logs and alerts on the target system**, potentially exposing the investigation.
...
🛡️ Phase 2: Google Hacking (Dorking)

- **Google Hacking Definition:** Using advanced search engine operators to find sensitive, hidden, or exposed information is known as **Google Hacking**.
    
- **Restricting Domains:** The `site:` operator is used to restrict search results to pages from a specific domain (e.g., `site:target.com`).
    
- **Searching URLs:** The `inurl:` operator is used to search for specific text patterns inside the URL.
    
- **Finding spreadsheets:** The dork operator `filetype:xlsx` is used to restrict search results to Excel spreadsheets.
    
- **Deleted Pages:** If you want to view a deleted or modified web page, you can use the Google operator `cache:`.
...
🔍 Phase 3: People & Username Recon

- **Username Lookup:** The command-line tool **Sherlock** is best for checking username reuse across 300+ sites simultaneously.
    
- **Email Harvesting:** The service **Hunter.io** helps you discover the email pattern format (e.g., `first.last@company.com`) used by an organization.
    
- **Breach Checking:** The service **HaveIBeenPwned** is used to check if an email address has been leaked in a known public data breach.
    
- **Facial Recognition:** The search engine **Yandex** is highly preferred and recommended for facial recognition checks.
...
⚡ Phase 4: Image & Infrastructure Mappings

- **Historic Snapshots:** The **Wayback Machine** provides historical snapshots of deleted or modified web pages.
    
- **GPS Metadata:** The **EXIF** metadata in a photograph provides GPS coordinates (latitude/longitude).
    
- **WiFi Mapping:** The tool **Wigle.net** allows you to search for WiFi hotspots by SSID or BSSID.
    
- **Domain Ownership:** The **WHOIS** record tells you who registered and owns a domain name.
    
- **Infrastructure Graphs:** The tool **DNSDumpster** visualizes a domain's DNS infrastructure, IP blocks, and subdomains as a visual graph.
    
- **Targeting Environments:** Subdomains like `dev` or `staging` are valuable targets because they **often have weaker security and configurations** compared to production servers.
    
- **Intelligence Steps:** The step in the Intelligence Cycle where you interpret processed data to find meaning is **Analysis**.
    
- **Grey Hat Sources:** Sites that provide paid access to raw leaked breach databases are considered **Grey Hat** OSINT sources.