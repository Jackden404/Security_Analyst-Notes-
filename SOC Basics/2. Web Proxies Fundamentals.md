...
Before you can manipulate web requests like an attacker, you need to be the man-in-the-middle. This is where Intercepting Proxies come into play.
...
---
##### 🛡️ Technical Overview: Intercepting Proxies vs. DevTools

Browsers have built-in Developer Tools (F12). So why do hackers use tools like Burp Suite or OWASP ZAP?
...
---
# Burp Suite Basics
Burp Suite is the single most important tool in a web pentester's arsenal.
...
##### 🛡️ Technical Overview: Configuring Your Browser
...
**Scenario:** In Burp's Proxy tab, what is the difference between Intercept and HTTP history? **Verdict:** **Intercept pauses individual requests in flight while HTTP history logs all traffic passively.** Most of the time, you will leave Intercept _off_ so your browsing is smooth, and review the traffic passively in the HTTP history tab. You only turn Intercept _on_ when you want to catch and alter a specific request right before it leaves your machine.

---
# Intercepting Requests

Intercepting proxies are not just passive monitors. They are active gatekeepers. In this module, we will dive into request interception, target parameter manipulation, and inspecting client-side response tokens.

🎯 Mission Objective

By the end of this module, you will understand when to use active request interception, how to discover mass assignment vulnerabilities, and how to analyze client-side session tokens. 
...
##### 🛡️ Technical Overview: Mass Assignment Vulnerability
...
1. Intercept a profile update request (`POST /api/profile`).
2. Inject extra fields to the JSON body: e.g., `"is_admin": true` or `"role": "admin"`.
3. Forward the request and check if your permissions changed. **Definition:** **Adding extra fields like is_admin to a request that the server processes and stores without validation.**
...
A security researcher's workflow is highly iterative. You capture a request, decode its components, modify parameters, and re-send it over and over. Burp Suite's Repeater and Decoder are built precisely for this cycle.
🎯 Mission Objective

By the end of this module, you will understand the manual testing cycle, how to test for blind vulnerabilities with Repeater, and how to bypass filters using double URL encoding.
...
##### 🛡️ Technical Overview: Bypassing Filters via Double URL Encoding
...
Anonymity and routing control are critical during penetration testing. If you scan a target directly from your home IP, you risk getting blacklisted or exposing your client's engagement details.
...
##### 🛡️ Technical Overview: Tor Exit Node Security Risks
